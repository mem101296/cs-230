...
@Path("/gameusers/") //added gameusers URL path
...
public class GameUserRESTController {
    ...
    @RolesAllowed({ "USER" }) //Allows only authenticated users roles to access this
    @GET
    @Path("/{id}")
    ...
    @RolesAllowed({ "ADMIN" }) //Allows only admin roles to access this.
    @POST
